...
__CHEETAH_genTime__ = 1211168791.9066861
...
''')
    ...
    def body(self, **KWS):
        ...
        write('''<h1>''')
        _v = VFFSL(SL,"title",True) # '$title' on line 12, col 5
        if _v is not None: write(_filter(_v, rawExpr='$title')) # from line 12, col 5.
        write('''</h1>
''')
        if VFFSL(SL,"message",True) != None and len(VFFSL(SL,"message",True)) > 0: # generated from line 13, col 1
            write('''<p>''')
            _v = VFFSL(SL,"message",True) # '$message' on line 14, col 4
            if _v is not None: write(_filter(_v, rawExpr='$message')) # from line 14, col 4.
            write('''</p>
''')
        write('''<form action="process_login" method="post">
  <p>
